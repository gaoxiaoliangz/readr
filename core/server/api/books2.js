...
const books = {
  ...
  // add(object, options) {
  //   let requiredOptions = ['bookContent', 'title', 'author', 'description', 'cover']
  //   let additionalOptions = ['tags', 'category', 'dbBookId']
  //
  //   const processDataAndDoQuery = (options) => {
  ...
  browse(options) {
    ...
    if(filter === 'user') {
      ...
      tasks.push(utils.requireUserPermissions)
    } else {
      doQuery = queryNewest
    }
